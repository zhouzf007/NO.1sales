...
public enum OrderStateEnum {
	...
	PAY("pay", "支付成功"), UNPAY("unpay", "未支付"), ACCEPT("accept", "制作中"), DONE("done", "待评价"), CONFIRM("confirm",
			"已完成"), REFUND("refund", "退款"), DELIVERY("delivery", "配送中"), DELIVERY_DONE("delivery_done",
					"配送完成"), CANCEL("cancel", "已取消"),
	/**
	 ...
	 * 根据英文名找中文名，不分大小写
	 */
	public boolean equals(String orderStateEn) {
		return this.orderStateEn.equals(orderStateEn);
	}

	public boolean equals(OrderStateEnum state) {
        return this.orderStateEn.equals(state.getOrderStateEn());
    }
}
